...
async function codeReview(bot, options, prompts) {
    ...
    if (options.max_files > 0 && files.length > options.max_files) {
        core.warning(`[Review] Limiting to ${options.max_files} files (${files.length} total)`);
